// gcc -Wall -Wextra -O3 attack.c -o attack
#include <stdlib.h>
#include <stdio.h>
#include <stdint.h>
#include <string.h>

const uint8_t S[7][4][256] = {
    {
...
const uint8_t invS_4_1[256] = {82, 9, 106, 213, 48, 54, 165, 56, 191, 64, 163, 158, 129, 243, 215, 251, 124, 227, 57, 130, 155, 47, 255, 135, 52, 142, 67, 68, 196, 222, 233, 203, 84, 123, 148, 50, 166, 194, 35, 61, 238, 76, 149, 11, 66, 250, 195, 78, 8, 46, 161, 102, 40, 217, 36, 178, 118, 91, 162, 73, 109, 139, 209, 37, 114, 248, 246, 100, 134, 104, 152, 22, 212, 164, 92, 204, 93, 101, 182, 146, 108, 112, 72, 80, 253, 237, 185, 218, 94, 21, 70, 87, 167, 141, 157, 132, 144, 216, 171, 0, 140, 188, 211, 10, 247, 228, 88, 5, 184, 179, 69, 6, 208, 44, 30, 143, 202, 63, 15, 2, 193, 175, 189, 3, 1, 19, 138, 107, 58, 145, 17, 65, 79, 103, 220, 234, 151, 242, 207, 206, 240, 180, 230, 115, 150, 172, 116, 34, 231, 173, 53, 133, 226, 249, 55, 232, 28, 117, 223, 110, 71, 241, 26, 113, 29, 41, 197, 137, 111, 183, 98, 14, 170, 24, 190, 27, 252, 86, 62, 75, 198, 210, 121, 32, 154, 219, 192, 254, 120, 205, 90, 244, 31, 221, 168, 51, 136, 7, 199, 49, 177, 18, 16, 89, 39, 128, 236, 95, 96, 81, 127, 169, 25, 181, 74, 13, 45, 229, 122, 159, 147, 201, 156, 239, 160, 224, 59, 77, 174, 42, 245, 176, 200, 235, 187, 60, 131, 83, 153, 97, 23, 43, 4, 126, 186, 119, 214, 38, 225, 105, 20, 99, 85, 33, 12, 125};
const uint8_t invS_2_0[256] = {82, 9, 58, 213, 102, 194, 225, 56, 245, 64, 163, 158, 129, 243, 215, 251, 190, 227, 57, 130, 155, 47, 8, 135, 52, 142, 67, 68, 196, 222, 233, 203, 84, 21, 148, 50, 253, 54, 35, 150, 238, 76, 149, 11, 66, 228, 185, 109, 255, 46, 161, 48, 40, 217, 36, 178, 19, 91, 162, 73, 78, 139, 209, 37, 114, 248, 246, 100, 249, 104, 117, 22, 212, 164, 92, 204, 93, 101, 182, 146, 108, 112, 202, 80, 166, 237, 195, 218, 169, 123, 63, 186, 167, 171, 157, 132, 144, 216, 141, 0, 28, 188, 211, 10, 247, 250, 88, 5, 184, 179, 69, 6, 208, 44, 105, 143, 72, 70, 15, 2, 193, 175, 189, 3, 153, 118, 138, 107, 106, 145, 17, 65, 79, 103, 220, 234, 151, 242, 207, 119, 229, 180, 230, 115, 61, 172, 116, 199, 231, 173, 53, 133, 226, 134, 55, 232, 140, 152, 223, 136, 71, 241, 26, 113, 29, 41, 83, 137, 111, 183, 98, 14, 170, 24, 124, 27, 252, 86, 62, 75, 198, 210, 121, 32, 154, 219, 192, 254, 120, 205, 90, 244, 31, 221, 168, 96, 110, 7, 34, 49, 177, 18, 16, 89, 39, 128, 236, 95, 51, 81, 127, 94, 60, 181, 74, 13, 45, 240, 122, 159, 147, 201, 156, 239, 160, 224, 59, 77, 174, 42, 191, 176, 99, 235, 187, 25, 131, 197, 1, 97, 23, 43, 4, 126, 87, 206, 214, 38, 165, 30, 20, 200, 85, 33, 12, 125};

void encrypt(const uint8_t *p, uint8_t *c,
            const uint8_t k0,
            const uint8_t k1,
            const uint8_t k2,
            const uint8_t k3,
    ...
    /* Round 0 */
    temp[3] = state[7] ^ S[0][3][state[0] ^ k2];
    temp[0] = state[4] ^ S[0][2][state[1] ^ k3];
    temp[1] = state[5] ^ S[0][1][state[2] ^ k4];
    temp[2] = state[6] ^ S[0][0][state[3] ^ k5];
    ...
    /* Round 1 */
    temp[3] = state[7] ^ S[1][3][state[0] ^ k0];
    temp[0] = state[4] ^ S[1][2][state[1] ^ k1];
    temp[1] = state[5] ^ S[1][1][state[2] ^ k2];
    temp[2] = state[6] ^ S[1][0][state[3] ^ k3];
    ...
    /* Round 2 */
    temp[3] = state[7] ^ S[2][3][state[0] ^ k4];
    temp[0] = state[4] ^ S[2][2][state[1] ^ k5];
    temp[1] = state[5] ^ S[2][1][state[2] ^ k0];
    temp[2] = state[6] ^ S[2][0][state[3] ^ k1];
    ...
    /* Round 3 */
    temp[3] = state[7] ^ S[3][3][state[0] ^ k2];
    temp[0] = state[4] ^ S[3][2][state[1] ^ k3];
    temp[1] = state[5] ^ S[3][1][state[2] ^ k4];
    temp[2] = state[6] ^ S[3][0][state[3] ^ k5];
    ...
    /* Round 4 */
    temp[3] = state[7] ^ S[4][3][state[0] ^ k0];
    temp[0] = state[4] ^ S[4][2][state[1] ^ k1];
    temp[1] = state[5] ^ S[4][1][state[2] ^ k2];
    temp[2] = state[6] ^ S[4][0][state[3] ^ k3];
    ...
    /* Round 5 */
    temp[3] = state[7] ^ S[5][3][state[0] ^ k4];
    temp[0] = state[4] ^ S[5][2][state[1] ^ k5];
    temp[1] = state[5] ^ S[5][1][state[2] ^ k0];
    temp[2] = state[6] ^ S[5][0][state[3] ^ k1];
    ...
    /* Round 6 */
    temp[3] = state[7] ^ S[6][3][state[0] ^ k2];
    temp[0] = state[4] ^ S[6][2][state[1] ^ k3];
    temp[1] = state[5] ^ S[6][1][state[2] ^ k4];
    temp[2] = state[6] ^ S[6][0][state[3] ^ k5];
...
void attack(const uint8_t* p, const uint8_t* c, 
            const uint8_t k2,
            const uint8_t k3,
            const uint8_t k4,
            const uint8_t k5,
            uint8_t *candidate_k0, uint8_t *candidate_k1) {

    uint8_t state[7][8];
    memcpy(state, p, 8);

    state[0][0] = p[4] ^ S[0][2][p[1] ^ k3];
    state[0][2] = p[6] ^ S[0][0][p[3] ^ k5];

    state[1][1] = p[1] ^ S[1][1][p[6] ^ S[0][0][p[3] ^ k5] ^ k2];
    state[4][2] = c[2] ^ S[6][3][c[7] ^ k5];

    state[3][6] = 
        c[2] ^ S[6][3][c[7] ^ k5]
        ^
        S[4][0][ 
            c[7] ^ S[5][0][c[0] ^ S[6][1][c[5] ^ k3] ^ k4]
            ^
            k3
        ]
    ;
    ...
    *candidate_k0 = state[4][2] ^ invS_4_1[ state[4][5] ^ c[5] ];
    
    state[1][3] = p[3] ^ S[1][3][ state[0][0] ^ *candidate_k0 ];

    *candidate_k1 =
        state[1][3]
        ^
        invS_2_0[ state[0][2] ^ state[3][6] ]
    ;
}
...
int main() {
    ...
    uint8_t p[8]       = {0xcc,0x7b,0x84,0x1c,0xeb,0xb5,0x69,0xe8};
    uint8_t c[8]       = {0xef,0x29,0x70,0xf2,0x17,0xd8,0xfc,0x32};
    uint8_t p_check[8] = {0x3e,0x0f,0x38,0x3f,0x48,0x5a,0xc1,0x9c};
    uint8_t c_check[8] = {0xe7,0x79,0x2f,0x5d,0x69,0x07,0xeb,0x2f};
    uint8_t c_test[8];
    ...
    for(k2 = 0x62; k2 <= 0xff; ++k2) { // Fast solution: start at 0x62
        printf("%02x\n", k2);
        for(k3 = 0; k3 <= 0xff; ++k3) {
            for(k4 = 0; k4 <= 0xff; ++k4) {
                for(k5 = 0; k5 <= 0xff; ++k5) {
                    attack(p, c, k2, k3, k4, k5, &k0, &k1);
                    encrypt(p_check, c_test, k0, k1, k2, k3, k4, k5);
                    if (memcmp(c_check, c_test, 8) == 0) {
                        printf("Key found!\n");
